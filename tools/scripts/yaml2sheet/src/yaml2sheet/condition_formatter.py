...
from .utils import get_result_column_index, column_index_to_letter


class ConditionFormatter:
    """Handles creation and formatting of conditional formulas for sheets"""

    def __init__(self, check_results: Dict[str, M17nField],
                 final_results: Dict[str, M17nField], target_id: str):
        ...
        result_column_index = get_result_column_index(target_id, COLUMNS)
        self.RESULT_COLUMN = column_index_to_letter(result_column_index)

    def get_condition_formula(self, cond: Condition,
                              id_to_row: Dict[str, int], lang: str) -> str:
        ...
        pass_formula = self.analyze_condition_formula(cond, id_to_row,
                                                      pass_phrase)
        # Unchecked state formula
        unchecked_formula = self.get_unchecked_formula(cond, id_to_row, lang)

        return (f'=IF({unchecked_formula},IF({pass_formula},'
                f'"{final_pass_phrase}","{final_fail_phrase}"))')
    ...
    def analyze_condition_formula(
        ...
        if cond['type'] == 'simple':
            ...
            return (f'TO_TEXT(${self.RESULT_COLUMN}${id_to_row[proc_id]})'
                    f'{operator}"{phrase}"')

        # Filter simple and complex conditions
        simple_conditions = [c for c in cond['conditions']
                             if c['type'] == 'simple']
        complex_conditions = [c for c in cond['conditions']
                              if c['type'] != 'simple']

        # Generate formulas
        simple_formulas = [
            self.analyze_condition_formula(c, id_to_row, phrase, type_,
                                           operator)
    ...
    def get_unchecked_formula(self, cond: Condition,
                              id_to_row: Dict[str, int], lang: str) -> str:
        ...
        row_count = len(rows)
        range_ = (f'${self.RESULT_COLUMN}${rows[0]}:'
                  f'${self.RESULT_COLUMN}${rows[row_count - 1]}')

        return f'COUNTIF({range_},"{unchecked_phrase}")={row_count},""'

    def get_relevant_rows(self, cond: Condition,
                          id_to_row: Dict[str, int]) -> List[int]:
