...
class CellData:
    ...
    def __init__(
        ...
        if (isinstance(value, str) and not value.strip() and
                type == CellType.PLAIN):
            self.value = None
        else:
            self.value = value
    ...
    def to_sheets_value(self) -> Dict:
        """Convert cell data to Google Sheets API format, ensuring empty cells
        are truly empty
        ...
        is_empty = (self.value is None or
        ...
        if is_empty:
            ...
            if self.type == CellType.PLAIN:
                result["userEnteredValue"] = {
                    "stringValue": str(self.value) if self.value is not None
                    else ""
                }
            elif self.type == CellType.RICH_TEXT:
                result["userEnteredValue"] = {
                    "stringValue": self.value.get('text', '')
                }
                    ...
                    result["textFormatRuns"] = [
                        {
                            ...
                            "format": {
                                ...
                                "foregroundColor": run['format'].get(
                                    'foregroundColor'
                                ),
