...
import logging
from .config import TARGET_NAMES, COLUMNS
from .sheet_structure import CheckInfo
from .utils import format_statement_summary
...
class DataProcessor:
    ...
    def process_source_data(self, src: Dict[str, Any]) -> Dict[str, List[Any]]:
        ...
        sorted_entries = sorted(src.items(),
                                key=lambda x: x[1].get('sortKey', 0))
        ...
        for key, check in sorted_entries:
            ...
            has_generated_data_flag = self._has_generated_data(
                check['sheetNames'])

            if not has_generated_data_flag:
    ...
    def _process_generated_data_fields(self, check: Dict) -> None:
        ...
        for statement in check.get('conditionStatements', []):
            ...
            check[column_header] = format_statement_summary(
                statement['summary'])
        ...
        for condition in check['conditions']:
            ...
            condition_target = (f"{check['target']}"
                                f"{condition['platform'].capitalize()}")
            ...
            if group_length == 1:
                ...
                self._process_multiple_procedures(check, condition,
                                                  procedures)
    ...
    def _process_multiple_procedures(self, check: Dict, condition: Dict,
        ...
        for proc in procedures:
            ...
            check['subchecks'][condition['target']]["conditions"].append(
                subcheck)
    ...
    def _record_check_info(self, check: Dict) -> None:
        ...
        if 'subchecks' in check:
            for target, subcheck_data in check['subchecks'].items():
                if (isinstance(subcheck_data, dict) and
                        'count' in subcheck_data):
    ...
    def _distribute_to_sheets(self, check: Dict,
                              processed_data: Dict[str, List]) -> None:
