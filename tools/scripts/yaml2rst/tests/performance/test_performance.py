...
class TestPerformance:
    ...
    def test_large_dataset_generation_performance(self, mock_templates):
            ...
            def generate(self):
                for i in range(self.item_count):
                    yield {
                        ...
                        'title': f'Item {i}',
                        'content': f'Content for item {i}' * 100,
                        'metadata': {
                            'id': f'item_{i}',
        ...
        assert execution_time < 10.0, (
            f"Generation took too long: {execution_time:.2f}s"
        )
    ...
    def test_generator_initialization_performance(self):
        ...
        assert avg_init_time < 0.01, (
            f"Generator initialization too slow: {avg_init_time:.4f}s"
        )
    ...
    def test_concurrent_generation_performance(self, mock_templates):
            ...
            def generate(self):
                for i in range(100):
                    yield {
                        ...
                        'content': (
                            f'Content from thread {self.thread_id}, item {i}'
                        ),
        ...
        assert total_time < 30.0, (
            f"Concurrent generation took too long: {total_time:.2f}s"
        )
        assert avg_individual_time < 10.0, (
            f"Individual generation too slow: {avg_individual_time:.2f}s"
        )
    ...
    def test_template_rendering_performance(self, mock_templates):
        ...
        assert rendering_time < 5.0, (
            f"Template rendering too slow: {rendering_time:.2f}s"
        )
    ...
    def test_selective_generation_performance(self, mock_templates):
        ...
        targets = [
            f'/tmp/test_output/file_{i:04d}.rst' for i in range(10)
        ]
    ...
    def test_error_handling_performance(self, mock_templates):
        ...
        assert error_handling_time < 10.0, (
...
class TestScalability:
    ...
    def test_generation_scales_linearly(self, mock_templates, item_count):
        """Test that generation time scales linearly with item count."""
        ...
        assert time_per_item < 0.1, (
            f"Time per item too high: {time_per_item:.4f}s"
        )

        # Verify all items were processed
        expected_count = item_count
        actual_count = mock_templates['category_page'].write_rst.call_count
    ...
    def test_memory_usage_scales_reasonably(self, mock_templates):
        ...
        for data_size in [1, 5, 10]:  # MB
            ...
            max_expected_memory = data_size * 3  # 3x overhead
            assert memory_increase < max_expected_memory, (
                f"Memory usage too high for {data_size}MB data: "
