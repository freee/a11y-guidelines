...
from yaml2rst.generators.common_generators import (
    ListBasedGenerator, SingleFileGenerator
)
...
class MockListBasedGenerator(ListBasedGenerator[str]):
    ...
    def __init__(self, lang: str, items: List[str] = None,
...
class MockSingleFileGenerator(SingleFileGenerator):
    ...
    def __init__(self, lang: str, template_data: Dict[str, Any] = None,
                 should_fail: bool = False):
        super().__init__(lang)
...
class TestListBasedGenerator:
    ...
    def test_get_items_not_implemented(self):
        """Test that get_items raises NotImplementedError."""
        # Create a subclass that implements process_item but calls
        # super() for get_items
    ...
    def test_process_item_not_implemented(self):
        ...
        # Create a subclass that implements get_items but calls
        # super() for process_item
        class IncompleteListGenerator(ListBasedGenerator[str]):
    ...
    def test_generate_processing_exception(self):
        ...
        with pytest.raises(ValueError,
                           match="Failed to process item: fail_item"):
            list(generator.generate())

        # Should have logged error
        generator.logger.error.assert_called_once()
        error_call_args = str(generator.logger.error.call_args)
        assert "Error processing item fail_item" in error_call_args
...
class TestSingleFileGenerator:
    ...
    def test_get_template_data_not_implemented(self):
        """Test that get_template_data raises NotImplementedError."""
        # Create a subclass that calls super() for
        # get_template_data
    ...
    def test_generate_template_data_exception(self):
        ...
        with pytest.raises(RuntimeError,
                           match="Failed to get template data"):
            list(generator.generate())

        # Should have logged error
        generator.logger.error.assert_called_once()
        error_call_args = str(generator.logger.error.call_args)
        assert "Error generating template data" in error_call_args
...
class TestGeneratorIntegration:
    ...
    def test_list_based_generator_inheritance(self):
        """Test that ListBasedGenerator inherits from BaseGenerator."""
        generator = MockListBasedGenerator('ja')
    ...
    def test_single_file_generator_inheritance(self):
        """Test that SingleFileGenerator inherits from BaseGenerator."""
        generator = MockSingleFileGenerator('ja')
    ...
    def test_error_handling_consistency(self):
        ...
        list_gen = MockListBasedGenerator('ja', ['fail_item'],
                                          should_fail=True)
...
class TestContentGeneratorBaseIntegration:
    ...
    def test_relationship_manager_access(self, mock_rm_class):
        """Test that generators can access RelationshipManager."""
    ...
    def test_validation_mixin_functionality(self):
        ...
        assert generator.validate_required_fields(
        ...
        assert generator.validate_string_field(
            data, 'filename', allow_empty=False
        ) is True

        empty_data = {'filename': ''}
        assert generator.validate_string_field(
            empty_data, 'filename', allow_empty=True
        ) is True
        assert generator.validate_string_field(
            empty_data, 'filename', allow_empty=False
        ) is False
    ...
    def test_utility_mixin_functionality(self):
        ...
        sorted_by_ja = generator.get_sorted_objects_by_lang_name(
            objects_with_names, 'ja'
        )
        ...
        template_data = generator.process_template_data(
            obj_with_template, 'ja'
        )
        assert template_data == {'lang': 'ja', 'data': 'test'}

        # Test process_template_data_list
        objects_with_template = [
            MockObjWithTemplateData(), MockObjWithTemplateData()
        ]
        template_data_list = generator.process_template_data_list(
            objects_with_template, 'ja'
        )
    ...
    def test_enhanced_validation_in_mock_generators(self):
        ...
        generator_invalid.process_item = Mock(
            return_value={'filename': '', 'content': 'test'}
        )
