...
def mock_category():
    ...
    mock_cat.get_dependency.return_value = ['data/yaml/gl/test_category/test.yaml']
    return mock_cat
...
class TestMakefileGenerator:
    ...
    def test_get_template_data_basic(self, mock_faq_class, mock_guideline_class, 
                                   mock_check_class, sample_makefile_config):
        """Test basic template data generation."""
        # Mock the class methods
        mock_check_class.list_all_src_paths.return_value = ['check1.yaml', 'check2.yaml']
        mock_guideline_class.list_all_src_paths.return_value = ['gl1.yaml', 'gl2.yaml']
        mock_faq_class.list_all_src_paths.return_value = ['faq1.yaml', 'faq2.yaml']
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.Category')
    def test_process_category_targets(self, mock_category_class, sample_makefile_config, mock_category):
        """Test category target processing."""
        mock_category_class.list_all.return_value = [mock_category]
        
        generator = MakefileGenerator('ja', sample_makefile_config)
        build_depends, category_targets = generator._process_category_targets()
        
        assert len(build_depends) == 1
        assert len(category_targets) == 1
        assert build_depends[0]['target'] == '/test/categories/test_category.rst'
        assert build_depends[0]['depends'] == 'data/yaml/gl/test_category/test.yaml'
        assert category_targets[0] == '/test/categories/test_category.rst'
    
    @patch('yaml2rst.generators.content_generators.makefile_generator.Category')
    def test_process_category_targets_duplicates(self, mock_category_class, sample_makefile_config):
        ...
        mock_category_class.list_all.return_value = [mock_cat1, mock_cat2]
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.CheckTool')
    def test_process_checktool_targets(self, mock_checktool_class, sample_makefile_config, mock_checktool):
        """Test checktool target processing."""
        mock_checktool_class.list_all.return_value = [mock_checktool]
        
        generator = MakefileGenerator('ja', sample_makefile_config)
        build_depends, checktool_targets = generator._process_checktool_targets()
        
        assert len(build_depends) == 1
        assert len(checktool_targets) == 1
        assert build_depends[0]['target'] == '/test/checks/examples/examples-test_tool.rst'
        assert build_depends[0]['depends'] == 'data/yaml/checks/test_tool.yaml'
        assert checktool_targets[0] == '/test/checks/examples/examples-test_tool.rst'
    
    @patch('yaml2rst.generators.content_generators.makefile_generator.CheckTool')
    def test_process_checktool_targets_duplicates(self, mock_checktool_class, sample_makefile_config):
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.FaqTag')
    @patch('yaml2rst.generators.content_generators.makefile_generator.Faq')
    def test_process_faq_targets(self, mock_faq_class, mock_faq_tag_class, 
                               sample_makefile_config, mock_faq, mock_faq_tag):
        ...
        build_depends, article_targets, tagpage_targets = generator._process_faq_targets()
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.FaqTag')
    @patch('yaml2rst.generators.content_generators.makefile_generator.Faq')
    def test_process_faq_targets_empty_tag(self, mock_faq_class, mock_faq_tag_class,
                                         sample_makefile_config, mock_faq, mock_faq_tag_empty):
        """Test FAQ target processing with empty tag (no articles)."""
        mock_faq_class.list_all.return_value = [mock_faq]
        mock_faq_tag_class.list_all.return_value = [mock_faq_tag_empty]
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.FaqTag')
    @patch('yaml2rst.generators.content_generators.makefile_generator.Faq')
    def test_process_faq_targets_duplicates(self, mock_faq_class, mock_faq_tag_class, sample_makefile_config):
        ...
        generator = MakefileGenerator('ja', sample_makefile_config)
        generator.relationship_manager.get_sorted_related_objects = Mock(return_value=[mock_faq1])
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.InfoRef')
    def test_process_info_targets_guidelines(self, mock_info_class, sample_makefile_config, mock_info_ref):
        ...
        build_depends, info_to_gl_targets, info_to_faq_targets = generator._process_info_targets()
        
        assert len(build_depends) == 1
        assert len(info_to_gl_targets) == 1
        assert len(info_to_faq_targets) == 0
        assert info_to_gl_targets[0] == '/test/info/test_ref.rst'
        assert build_depends[0]['depends'] == 'gl_src.yaml'
    
    @patch('yaml2rst.generators.content_generators.makefile_generator.InfoRef')
    def test_process_info_targets_faqs(self, mock_info_class, sample_makefile_config, mock_info_ref):
        ...
        assert info_to_faq_targets[0] == '/test/info/test_ref.rst'
        assert build_depends[0]['depends'] == 'faq_src.yaml'
    
    @patch('yaml2rst.generators.content_generators.makefile_generator.InfoRef')
    def test_process_info_targets_external_skipped(self, mock_info_class, sample_makefile_config, 
    ...
    @patch('yaml2rst.generators.content_generators.makefile_generator.InfoRef')
    def test_process_info_targets_duplicates(self, mock_info_class, sample_makefile_config):
        ...
        mock_info_class.list_has_guidelines.return_value = [mock_info1, mock_info2]
        mock_info_class.list_has_faqs.return_value = []
    ...
    def test_validate_data_missing_required_fields(self, sample_makefile_config):
    ...
    def test_validate_data_invalid_depends_structure(self, sample_makefile_config):
