...
class TestTemplateManager:
    ...
    def test_init(self):
        ...
        with patch('yaml2rst.template_manager.Environment') as mock_env_class, \
             patch('yaml2rst.template_manager.FileSystemLoader') as mock_loader_class:
    ...
    def test_write_rst(self):
        ...
        with patch('yaml2rst.template_manager.Environment'), \
            ...
            mock_file.assert_called_once_with(output_path, mode='w', encoding='utf-8', newline='\n')
    ...
    def test_make_heading_mixed_characters(self):
        """Test heading generation with mixed ASCII and multibyte characters."""
        title = "Test テスト"  # Mixed English and Japanese
        level = 3
        
        result = TemplateManager.make_heading(title, level)
        
        # "Test " = 5 ASCII chars (width 5) + "テスト" = 3 Japanese chars (width 6) = total width 11
        expected = "Test テスト\n==========="
        assert result == expected
    ...
    def test_make_heading_invalid_level_too_low(self):
        ...
        with pytest.raises(ValueError, match="Invalid level: 0. Must be between 1 and 6"):
    ...
    def test_make_heading_invalid_level_too_high(self):
        ...
        with pytest.raises(ValueError, match="Invalid level: 7. Must be between 1 and 6"):
            TemplateManager.make_heading(title, level)
