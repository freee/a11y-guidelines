...
class TestConfig:
    ...
    @patch('yaml2rst.config.Config.get_available_languages')
    def test_get_available_languages_exception_fallback(self, mock_config_method):
        """Test fallback behavior when Config.get_available_languages() raises an exception."""
    ...
    @patch('yaml2rst.config.Config.get_available_languages')
    def test_get_available_languages_attribute_error_fallback(self, mock_config_method):
        """Test fallback behavior when Config doesn't have the method."""
        # Setup mock to raise AttributeError
        mock_config_method.side_effect = AttributeError("'Config' object has no attribute 'get_available_languages'")
    ...
    def test_get_available_languages_import_error_fallback(self, mock_config_class):
        """Test fallback behavior when Config import fails."""
        # Setup mock to raise ImportError when accessing get_available_languages
        mock_config_class.get_available_languages.side_effect = ImportError("Cannot import Config")
    ...
    def test_available_languages_constant_initialization(self):
        """Test that AVAILABLE_LANGUAGES is initialized with get_available_languages()."""
        # This test verifies that the constant is properly initialized
        # We don't need to mock since we're testing the actual initialization behavior
        from yaml2rst.config import AVAILABLE_LANGUAGES, get_available_languages
    ...
    def test_backward_compatibility(self):
        ...
        # They should be equivalent (though not necessarily the same object)
