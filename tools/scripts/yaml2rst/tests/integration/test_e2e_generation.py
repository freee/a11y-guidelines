...
class TestEndToEndGeneration:
    ...
    def test_complete_generation_workflow(
        ...
        mock_guideline1.template_data.return_value = {'id': 'form_001', 'title': 'Form Labels', 'priority': 'high'}
        
        mock_guideline2 = Mock()
        mock_guideline2.template_data.return_value = {'id': 'form_002', 'title': 'Form Validation', 'priority': 'medium'}
        
        mock_guideline3 = Mock()
        mock_guideline3.template_data.return_value = {'id': 'image_001', 'title': 'Alt Text', 'priority': 'high'}
    ...
    def test_file_generator_with_real_category_generator(self, temp_dir, mock_templates):
    ...
    @patch('freee_a11y_gl.Category.list_all')
    @patch('freee_a11y_gl.relationship_manager.RelationshipManager.get_sorted_related_objects')
    def test_category_generator_integration(self, mock_get_related_objects, mock_list_all, temp_dir, mock_templates):
        ...
        mock_get_related_objects.return_value = [mock_guideline1, mock_guideline2]
    ...
    def test_error_handling_in_generation_pipeline(self, temp_dir, mock_templates):
    ...
    def test_multiple_generators_workflow(self, temp_dir, mock_templates):
        ...
        class CategoryGen:
            ...
            def generate(self):
                yield {'filename': 'categories', 'type': 'category', 'content': 'Category content'}
        ...
        class FaqGen:
            ...
            def generate(self):
                yield {'filename': 'faq', 'type': 'faq', 'content': 'FAQ content'}
        ...
        # Each generator should be called once, but category_page might be used by both
        # so we check that at least one call was made to each template
        ...
        # Note: category_page template might be called multiple times if used by different generators
        assert category_template.write_rst.call_count >= 1
        assert faq_template.write_rst.call_count >= 1
        ...
        category_found = any(call[0][0]['type'] == 'category' for call in category_calls)
        faq_found = any(call[0][0]['type'] == 'faq' for call in faq_calls)
        
        assert category_found, "Category type content not found in category template calls"
        assert faq_found, "FAQ type content not found in FAQ template calls"
