...
class TestCLIInterface:
    ...
    def test_cli_default_arguments(
        ...
        with patch('yaml2rst.initializer.get_dest_dirnames') as \
             mock_get_dest, \
             patch('yaml2rst.initializer.get_static_dest_files') as \
             mock_get_static, \
             patch('freee_a11y_gl.source.get_src_path') as mock_get_src, \
             patch('yaml2rst.initializer.TemplateManager') as \
             mock_template_mgr:
            ...
            mock_get_static.return_value = {
                ...
                'faq_article_index': str(temp_dir / 'faq' / 'articles' /
                                         'index.rst'),
            ...
            mock_get_src.return_value = {
                'wcag_sc': str(temp_dir / 'wcag.json')
            }
            ...
            default_lang = (config_call['config_override']['languages']
                            ['default'])
    ...
    def test_cli_custom_basedir(
        ...
        with patch('yaml2rst.initializer.get_dest_dirnames') as \
            ...
            with patch('sys.argv', ['yaml2rst', '--basedir',
                                    str(custom_basedir)]):
                yaml2rst.main()
            ...
            mock_config.initialize.assert_called_once()
            config_call = mock_config.initialize.call_args[1]
            basedir = config_call['config_override']['basedir']
            assert str(custom_basedir) in basedir
    ...
    def test_cli_specific_files(
        ...
        with patch('yaml2rst.initializer.get_dest_dirnames') as \
            ...
            args = ['yaml2rst', str(test_file1), str(test_file2)]
            with patch('sys.argv', args):
                yaml2rst.main()

            # Verify that generate was called with build_all=False and targets
    ...
    def test_cli_argument_parsing_build_all(self):
        ...
        args = ['yaml2rst', '--lang', 'ja', '--basedir', '/test']
        with patch('sys.argv', args):
    ...
    def test_cli_short_arguments(self, mock_get_languages):
        ...
        with patch('sys.argv', ['yaml2rst', '-l', 'en', '-b',
                                '/custom/base']):
            args = initializer.parse_args()
...
class TestCLIErrorHandling:
    ...
    def test_cli_directory_creation_error(self, mock_makedirs):
        ...
        with patch('yaml2rst.yaml2rst.Config'), \
             patch('yaml2rst.yaml2rst.setup_instances'), \
             patch('yaml2rst.initializer.get_dest_dirnames') as \
             mock_get_dest, \
             patch('yaml2rst.initializer.get_static_dest_files') as \
             mock_get_static, \
             patch('freee_a11y_gl.source.get_src_path') as mock_get_src, \
...
class TestCLIIntegration:
    ...
    def test_cli_full_workflow(
        ...
        with patch('yaml2rst.initializer.get_dest_dirnames') as \
            ...
            mock_get_static.return_value = {
                ...
                'faq_tag_index': str(output_dir / 'faq' / 'tags' /
                                     'index.rst'),
                'faq_article_index': str(output_dir / 'faq' / 'articles' /
                                         'index.rst'),
                'wcag21mapping': str(output_dir / 'info' /
                                     'wcag21-mapping.rst'),
    ...
    def test_cli_argument_validation(self):
        ...
        with patch('yaml2rst.initializer.config.get_available_languages') \
                as mock_langs:
